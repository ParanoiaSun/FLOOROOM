...
public class JPanelGame extends JPanel{
	...
	public void initialText(){
		...
		Font textFont = new Font("Calibri (正文)",Font.PLAIN,30);
		Color textColor = new Color(105,63, 47);
		countText.setForeground(textColor);
		countText.setFont(textFont);
		add(countText);
		countText.setBounds(50, 0, 400, 130);		
	...
	public void directAdd(Person person,int i){//添加上下左右按钮,i用于表示执行的具体命令
		...
		if(commandNow==2){
			...
			}if(control.personNow.location.x==14||(control.blocks[control.personNow.location.x+1][control.personNow.location.y].hasPerson()&&!control.personNow.isHide())){
				buttons[7].setVisible(false);
			}if(control.personNow.location.y==0||(control.blocks[control.personNow.location.x][control.personNow.location.y-1].hasPerson()&&!control.personNow.isHide())){
				buttons[4].setVisible(false);
			}if(control.personNow.location.y==14||(control.blocks[control.personNow.location.x][control.personNow.location.y+1].hasPerson()&&!control.personNow.isHide())){
